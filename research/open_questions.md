...
| Q-0004 | Daytona sandbox API key — do we need one, or does TrueForge's local sandbox fallback satisfy the "code runs in a sandbox" judging beat? | 2026-08-27 | **Answered 2026-08-27, still blocked** | We need a Daytona key. `SandboxProviderManifest.type` is the single-value enum `["daytona"]` — there is no configurable local provider, whatever the boot log's "local sandbox fallback" line suggests. `GET /settings/sandbox-providers` returns 404 until one is set, and **skills require a sandbox**, so this blocks two judged features at once. |
