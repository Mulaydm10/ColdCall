...
| 2026-08-27 | `COMPETITION.md` | Corrected the San Francisco day from past tense to upcoming (2026-08-29, separate Luma registration) | Mulaydm10 (via Claude, on their behalf) | Fixes Qodo finding #1 on PR #1 |
| 2026-08-27 | `tests/README.md` | Rewritten: the green baseline ADR-0002 required now exists; documents commands, markers, and the no-transcribed-constants rule | Mulaydm10 (via Claude, on their behalf) | Previous text ("no runnable test baseline yet") was stale on merge of the stack work |
| 2026-08-28 | `VISION.md` | Applied `proposals/VISION.md` verbatim, replacing the all-TODO scaffold with the real thesis | Mulaydm10 (via Devin, on their behalf — explicit instruction in chat) | Resolves Q-0001; the proposal itself is unchanged and remains in `proposals/` for the diff trail |
