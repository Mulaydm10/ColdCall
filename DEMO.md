...
cp .env.example .env                       # fill in the THREE keys named below
uv sync --group dev && uv run pytest       # 230 green — the maths, before you trust it
./scripts/verify_apis.sh                   # every external source, hit for real
./scripts/setup_trueforge.sh               # expect: 5 configured, 0 failed
```

**Three keys are required for this scenario**, and `setup_trueforge.sh` *skips* rather than
...
- **`GITHUB_TOKEN`** — without it the GitHub connector is skipped, and **steps 6 and 7 have no
  tool to call**: the approval gate never fires and nothing is committed. This is the one that
  silently removes the demo's centrepiece rather than failing loudly, so check for it
  specifically. A classic PAT with `repo` scope, or `gh auth token` if you already use the
  GitHub CLI.

If the script says `skip` next to any of the three, stop and fix that before going further —
`5 configured, 2 skipped, 0 failed` is the expected line, with Supabase and Stripe the two
skips.
...
| 4 | **The maths, off-host** | The orchestrator clones the repo **inside a Daytona microVM** and runs the deterministic module itself, before it fans out — the verdict is the thing everything else depends on, so it is not delegated. Show the verdict JSON: MKT **24.54 °C**, budget **64.35 %**, verdict **`quarantine_retest`** — and the `cross_check` block showing the same number reached by a second, different implementation. Say: *this is WHO TRS-999 and USP <1079> arithmetic, in the repo, unit-tested — not an LLM guess. Same numbers on my laptop.* | 1:05–1:40 |
| 5 | **Why it warmed** | Show the `route_context` block. The Route Analyst pulled **the ERA5 weather archive for this shipment's last-known GPS position**: across the matched readings the outside air peaked at **17.3 °C** while the box hit **27 °C** — a **12.6 °C** median gap, 14 of 14 readings matched. Say: *this was not a hot day. The weather does not account for it, so this is a containment failure — the investigation goes to the packaging and the reefer, not to the lane.* Then read the qualifier off the screen: *and the record says what that rests on — the GPS fixes predate the excursion by 18.9 hours, so this is marked qualified.* **If this take's route lookup failed** (an in-sandbox network flake, seen twice in rehearsal), the `route_context` block will carry an `error` instead — skip to the fallback under *Known-broken edges*, keep the verdict, and do not improvise a cause. | 1:40–2:00 |
...
5. **"The weather archive is real; the coordinate is a last-known position."** ERA5 is a
   reanalysis — an observation-constrained model, not a thermometer at that spot. And the GPS
   predates the excursion by 18.9 h, which is why the record marks the attribution
   `qualified`. Say both if asked; the numbers are on screen. The GPS comes from the dataset's own
