...
function Stage({
  ...
  playback = null,
  // Transport chrome (the playback bar). 'none'/'false' hides it for
  // product surfaces where the animation should read as content, not an
  // editor. Hiding it changes nothing about timing or the seek protocol.
  chrome = true,
  ...
  if (typeof chrome === 'string') chrome = chrome !== 'none' && chrome !== 'false';
  ...
  React.useEffect(() => {
    ...
    passesRef.current = 0;
    const step = (ts) => {
      // Skip work while the stage is hidden (route not visible) so the
      // scene doesn't keep rendering offscreen.
      const el = stageRef.current;
      if (!el || el.offsetParent === null || document.hidden) {
        lastTsRef.current = null;
...
function CompositionStage(props) {
  ...
  var chrome = props.chrome == null ? true : (String(props.chrome) !== 'none' && String(props.chrome) !== 'false');
  ...
  var scenes = React.useMemo(function () {
    return ssParse(raw) || ssParse(typeof window !== 'undefined' ? window.OM_SCENES : null);
  }, [raw]);
  ...
  return (
    <React.Fragment>
      <Stage width={width} height={height} duration={derived.total} background={bg}
             autoplay={autoplay} loop={loop} playback={pb} chrome={chrome}>
